...
export default function Overview({ calls, sales, allCalls, allSales, role, scope, scopeLabel }) {
  ...
  // AI analysis: ONE shared desk snapshot for everyone, generated by the
  // admin from ALL traders' data and published to the cloud. Local cache of
  ...
  useEffect(() => {
    if (!isAdmin) return
    let active = true
    weeklyReadiness().then(r => { if (active) setReadiness(r) }).catch(() => {})
    return () => { active = false }
  }, [isAdmin, aiLoading])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const snap = await loadLatestAnalysisSnapshot()
        if (active && snap) {
          setAnalysis(snap)
          try { localStorage.setItem('fertintel_ai_analysis:global', JSON.stringify(snap)) } catch {}
        }
      } catch { /* cloud unreachable: keep local cache */ }
    })()
    return () => { active = false }
  }, [])
    ...
    <div className={styles.wrap}>
      ...
      <section className={styles.section}>
        <div className={styles.aiHeader}>
          ...
          <div className={styles.aiHeaderRight}>
            ...
            {isAdmin ? (
              <>
              {readiness && (
                <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: 'var(--text3)', marginRight: 12 }}>
                  This week:{' '}
                  {readiness.map((r, i) => (
                    <span key={r.label} style={{ color: r.ok ? 'var(--accent)' : 'var(--text3)' }}>
              ...
              <span className={styles.aiTimestamp}>Desk analysis · all traders · generated by admin</span>
            )}
          </div>
        </div>
        ...
        {!snapshotIsCurrent && analysis?.weekLabel && !aiLoading && (
          <p className={styles.staleNote}>
            Showing last locked read ({analysis.weekLabel}).{isAdmin ? " Generate this week's analysis when the desk's calls and publications are in." : " This week's desk analysis hasn't been generated yet."}
          </p>
        )}
        ...
        {!aiLoading && signals.length === 0 && !aiError && (
          <div className={styles.aiLoading}>
            <p>{deskCalls.length === 0 ? 'Log calls to generate market signals.' : isAdmin ? 'No analysis yet — click Generate to analyze this week.' : 'No desk analysis published yet.'}</p>
          </div>
        )}
          ...
          <div className={styles.signal} style={{
            ...
            <div className={styles.signalRow}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {analysis.stanceLockedAt && (
                  <p style={{ color: 'var(--text3)', fontSize: 11, marginTop: 4, fontFamily: "'DM Mono', monospace" }}>
                    ◆ Definitive stance for this week — locked {new Date(analysis.stanceLockedAt).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })} · later regenerations update signals & analysis only
                  </p>
                )}
      ...
      {analysis?.analysis && (
        <section className={styles.section}>
          ...
          <p className={styles.aiDisclaimer}>Desk-wide analysis (all traders) · Generated by Claude Fable 5 · Informational, not financial advice</p>
        </section>
      )}
